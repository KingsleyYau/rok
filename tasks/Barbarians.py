...
class Barbarians(Task):
    ...
    def set_barbarians_level(self, level):
        ...
        _, _, dec_pos = self.gui.check_any(
            ImagePathAndProps.DECREASING_BUTTON_IMAGE_PATH.value
        )
        has_inc_btn, _, inc_pos = self.gui.check_any(
            ImagePathAndProps.INCREASING_BUTTON_IMAGE_PATH.value
        )
        _, _, lock_pos = self.gui.check_any(
            ImagePathAndProps.LOCK_BUTTON_IMAGE_PATH.value
        )
        ...
        if max_lv != -1:
            x = (
                max_pos[0]
                if level > max_pos[0] or level >= 99
                else ((level - base_level) / (max_lv - base_level))
                * (max_pos[0] - min_pos[0])
                + min_pos[0]
            )
            super().tap(max_pos[1])
    ...
    def hold_pos_after_attack(self, should_hold):
        is_check, _, pos = self.gui.check_any(
            ImagePathAndProps.HOLD_POS_CHECKED_IMAGE_PATH.value
        )
    ...
    def battle_result_detector(self, commander_cv_img):
        ...
        while True:
            ...
            super().set_text(
                replace="Waiting: {}/{}".format(
                    int(time_eclipsed), self.bot.config.timeout
                ),
                index=0,
            )
            if found:
    ...
    def wait_for_commander_back_to_city(self, commander_cv_img):
        ...
        while True:
            ...
            time_eclipsed = time.time() - start
            super().set_text(
                replace="Wait Commander return to City: {}/{}".format(
                    int(time_eclipsed), self.bot.config.timeout
                ),
                index=0,
            )
    ...
    def use_ap_recovery(self):
        ...
        if name == WINDOW:
            if self.bot.config.useDailyAPRecovery:
                _, _, pos = self.gui.check_any(
                    ImagePathAndProps.DAILY_AP_CLAIM_BUTTON_IMAGE_PATH.value
                )
                if pos is not None:
            ...
            if self.bot.config.useNormalAPRecovery and not used:
                _, _, pos = self.gui.check_any(
                    ImagePathAndProps.USE_AP_BUTTON_IMAGE_PATH.value
                )
                if pos is not None:
